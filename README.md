...
Use `aws.creator` and `aws.creator_name` to tag who originally created the Mac. `cm` writes `cm-creator-date` automatically from the creation date, so new profiles should not set `aws.creator_date`. Use `aws.account_email` for the Apple account email. Leave `aws.resource_name` empty for new resources so `cm` generates `xcode-<account-email>`. Set `aws.resource_name` and `aws.creator_date` only when adopting resources that were created before `cm` managed them.
