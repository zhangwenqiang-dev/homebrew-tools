...
func DefaultRulesTemplate() string {
	return strings.Join([]string{
		...
		"9. A pending but empty Dedicated Host may be previewed and attempted with explicit confirmation only after the previously associated EC2 instance has terminated and no longer appears on that host, usually after about 1 hour. If AWS rejects RunInstances, report that exact reason and stop.",
...
func DefaultSkillTemplate() string {
	return strings.Join([]string{
		...
		"- A pending but empty Dedicated Host may be previewed and attempted with explicit confirmation only after the previously associated EC2 instance has terminated and no longer appears on that host, usually after about 1 hour. If AWS rejects RunInstances, report that exact reason and stop.",
		"- Run GUI/VNC setup only after AWS readiness checks pass. Ask the user for the ec2-user password, never invent it, never write it to config/logs/output, and use it only for that setup operation.",
		...
		"- `cm_aws_setup_gui`: after readiness, set the ec2-user password for this operation only and enable macOS Screen Sharing.",
		"",
