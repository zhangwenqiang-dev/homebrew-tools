...
func DefaultRulesTemplate() string {
	return strings.Join([]string{
		...
		"- AWS status/open/destroy tools return text plus structuredContent fields such as profile, apple_email, decision, next, confirmed, ready, and eip_retained.",
...
func DefaultSkillTemplate() string {
	return strings.Join([]string{
		...
		"- `cm_dashboard`: show profile/tunnel dashboard, with optional read-only AWS decision and next-step columns.",
		"- `cm_push`: preview or execute rsync upload with optional `includes`/`excludes`; requires `confirm=true` to execute.",
		"- `cm_pull`: preview or execute rsync download with optional `includes`/`excludes`; requires `confirm=true` to execute.",
		...
		"- `cm_aws_open_mac_by_email`: preview or open by explicit `apple_email`; requires `confirm=true` to mutate AWS; returns structuredContent.",
